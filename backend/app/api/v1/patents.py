...
async def create_patent(
    ...
    return await PatentService(db).create_with_audit(data, current_user.id, ip)
...
async def update_patent(
    ...
    ip = request.client.host if request.client else None
    return await PatentService(db).update_with_audit(patent_id, data, current_user.id, ip)
